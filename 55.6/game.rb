class Game                #class Game
 ...
 #def actions
  ...
  def check_info
    ...
    show_score(@dealer.name, @dealer.score(@dealer.cards)) if @game_over
    show_score(@player.name, @player.score(@player.cards))
    show_bank(@bank)
  end
  ...
  def determine_winner
    player_score = @player.score(@player.cards)
    dealer_score = @dealer.score(@dealer.cards)
