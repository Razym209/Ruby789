...
class Game
  ...
  def new_game_now
    case @main.show_base_commands
    when 1 then start_game
    when 2 then main.exit_game
    end
  end
  ...
  def show_info
    ...
    main.show_score(@dealer.name, @hand.score(@dealer.cards)) if @game_over
    main.show_score(@player.name, @hand.score(@player.cards))
    main.show_bank(@bank)
  end
